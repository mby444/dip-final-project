...
def process_image(category_name, image_name, image_path):
    ...
    # =====================================================
    # LOGIKA MENENTUKAN METODE TERBAIK
    # =====================================================
    metode_data = [
        {"nama": "Sobel", "mse": mse_sobel, "psnr": psnr_sobel},
        {"nama": "Prewitt", "mse": mse_prewitt, "psnr": psnr_prewitt},
        {"nama": "Canny", "mse": mse_canny, "psnr": psnr_canny}
    ]
    
    # Metode terbaik didasarkan pada PSNR tertinggi
    metode_terbaik = max(metode_data, key=lambda x: x["psnr"])

    # =====================================================
    # PRINT HASIL (TAMPILAN TABEL & KESIMPULAN)
    # =====================================================
    print("\nHASIL EVALUASI:\n")
    
    # Header Tabel
    print("+" + "-"*12 + "+" + "-"*14 + "+" + "-"*14 + "+")
    print(f"| {'Metode':^10} | {'MSE':^12} | {'PSNR':^12} |")
    print("+" + "-"*12 + "+" + "-"*14 + "+" + "-"*14 + "+")
    
    # Isi Tabel
    print(f"| {'Sobel':<10} | {mse_sobel:>12.2f} | {psnr_sobel:>12.2f} |")
    print(f"| {'Prewitt':<10} | {mse_prewitt:>12.2f} | {psnr_prewitt:>12.2f} |")
    print(f"| {'Canny':<10} | {mse_canny:>12.2f} | {psnr_canny:>12.2f} |")
    
    # Footer Tabel
    print("+" + "-"*12 + "+" + "-"*14 + "+" + "-"*14 + "+")
    
    # Output Kesimpulan
    print(f">>> KESIMPULAN: Metode terbaik adalah ** {metode_terbaik['nama'].upper()} **")
    print(f"    (PSNR Tertinggi: {metode_terbaik['psnr']:.2f} dB | MSE Terendah: {metode_terbaik['mse']:.2f})\n")
